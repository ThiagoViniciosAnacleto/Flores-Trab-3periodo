...
if (hamburger && navbar && carrosDestaqueTitle) {
    hamburger.addEventListener("click", function () {
        navbar.classList.toggle("active"); // Alterna a classe 'active' ao clicar no hamburger
        navIcons.classList.toggle("active");

        // Ajusta a margem superior do título para movê-lo para baixo
        if (navbar.classList.contains("active")) {
            carrosDestaqueTitle.style.marginTop = "75px"; // Ajuste o valor conforme necessário
        } else {
            carrosDestaqueTitle.style.marginTop = "0"; // Restaura o valor original
        }
    });
} else {
